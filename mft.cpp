...
void mft_best_fit()
{
    ...
    static vector<int> v;
    //function to print the final table of partitions
    auto print=[](){
        int extfrag = 0;
...
struct Partition {
    ...
    // Helper function to insert a process into the array of partitions
auto insert = [] (int pid, int size, int idx) {
    partitions[idx].pid = pid;
    partitions[idx].psize = size;
    // If the partition size is greater than the size of the process, increase internal fragmentation
    if (partitions[idx].size > size) {
        int_frag += partitions[idx].size - size;
    }
    // Update the value of number of processes
    n_proc++;

    cout << "Process " << pid <<  " allocated to Partition " << idx << " successfully!\n";
};

// Function to process the size of each partition
auto inputPartition=[] () {
    int i, off = 0;
...
// Function to insert a process to the buffer using Worst Fit
auto worstFit = [](int pid, int size) {
    ...
    //insert(pid, size, worst_idx);
    //(int pid, int size, int idx)
...
// Function to delete a process from the buffer
auto deleteProc= [] (int pid) {
